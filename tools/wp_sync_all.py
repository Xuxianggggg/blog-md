...
def upsert_post(wp_url, auth, md_path: str, status: str, managed_tag_id: int):
    ...
    if old_post:
        ...
        return j.get("slug", slug)  # 返回 WordPress 实际 slug
    else:
        r = requests.post(
            f"{wp_url}/wp-json/wp/v2/posts",
            auth=auth,
...
def main(status: str, force_delete: bool):
    ...
    for md_file in repo_md_files:
        print(" -", md_file)
        real_slug = upsert_post(wp_url, auth, md_file, status, managed_tag_id)
        desired_slugs.add(real_slug)

    # 2) 再删除 WordPress 里那些仓库中已不存在的“受管理文章”
